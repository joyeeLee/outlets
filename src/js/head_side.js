...
Listpage.prototype.calculate = function () {//验证码计算
...
Listpage.prototype.logincheck = function () {//登录跳转
    // var _this = this;
    $.ajax({
        type: 'post',
        url: '../api/login.php',
        ...
        success: function (str) {
            if (str == 1) {
                ...
                if ($('.checkcode').val() == $('.cal_res').html()) {
                    ...
                    // $('.logingo').html($('.inp1_val').val());
                    $('.logingo').html('.inp1_val'.val()).attr("href", "#");

                    // $('.free_reg').html('退出');
                    $('.free_reg').html("退出").attr("href", "login.html");
...
Listpage.prototype.totop = function () {//回到顶部的方法
...
Listpage.prototype.isgetcookie = function () {
